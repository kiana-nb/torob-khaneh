...
export function Gallery({ images, title, stock }: { images: string[]; title: string; stock?: boolean }) {
  ...
  return (
    <>
      ...
      {/* desktop: mosaic */}
      <div className={cn('relative hidden gap-2 overflow-hidden rounded-sheet md:grid', images.length >= 5 ? 'h-[440px] grid-cols-4 grid-rows-2' : images.length >= 3 ? 'h-[420px] grid-cols-3 grid-rows-2' : 'h-[420px] grid-cols-2')}>
        {images.slice(0, 5).map((src, i) => (
          <button key={src} onClick={() => show(i)} className={cn('group relative overflow-hidden bg-surface-3', i === 0 && (images.length >= 3 ? 'col-span-2 row-span-2' : 'row-span-1'))}>
